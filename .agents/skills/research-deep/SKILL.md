...
**Suggest the ingest pass.** Once the raw notes exist, propose running the ingest workflow in `docs/agent/workflows.md` next — distilling the new findings into the relevant idea overview and deep-dive sections, merging with existing synthesis instead of appending duplicates, regenerating `wiki/index.md`, updating `.manifest.json` and `hot.md`, and appending to `wiki/log.md`. Offer to do it directly. Don't leave new evidence as raw-only.
